...
def parse_solution_text_format(solution_text: str) -> Dict[str, str]:
    ...
    for line in solution_text.split('\n'):
        ...
        match = re.search(r'\b([A-Za-z]+)\b\s+is\s+(reachable|unreachable)\b', line, re.IGNORECASE)
        if match:
...
def parse_model_answer(answer_text: str, expected_names: list) -> Optional[Dict[str, str]]:
    ...
    # 计数
    true_count = len(re.findall(r'\bis\s+reachable\b', answer_text, re.IGNORECASE))
    false_count = len(re.findall(r'\bis\s+unreachable\b', answer_text, re.IGNORECASE))
    ...
    for name in expected_names:
        pattern = re.compile(
            rf'\b{name}\b\s+is\s+(reachable|unreachable)\b', 
            re.IGNORECASE
        )
